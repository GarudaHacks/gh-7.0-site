  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#0F0622]/90 backdrop-blur-md overflow-hidden perspective-1000">
      ...
      <button
        ...
        className="absolute top-6 right-6 z-50 w-12 h-12 flex items-center justify-center rounded-full border border-[#C4A9FF]/40 text-[#C4A9FF] bg-[#1E0A3C] text-lg cursor-pointer transition-all duration-200 hover:bg-[#C4A9FF] hover:text-[#221139] hover:rotate-90 shadow-lg"
      >
      ...
      {(launchState === "idle" || launchState === "shaking" || launchState === "launching") && (
        ...
        >
          <img
            ...
            alt="Cockpit panel"
            className="w-full md:w-[min(90vw,900px)] h-auto pointer-events-none select-none drop-shadow-2xl"
          />
          ...
          {launchState === "shaking" && (
            <div className="absolute inset-x-0 -bottom-20 h-64 flex items-center justify-center pointer-events-none overflow-visible z-[-1]">
              <div className="smoke-cloud w-64 h-64" style={{ left: '30%', animationDelay: '0s' }} />
              <div className="smoke-cloud w-80 h-80" style={{ left: '50%', animationDelay: '0.2s' }} />
              <div className="smoke-cloud w-64 h-64" style={{ left: '70%', animationDelay: '0.1s' }} />
            </div>
          )}
          ...
          {launchState === "launching" && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none origin-center z-50">
              ...
              {/* Flash putih buat transisi mulus ke halaman berikutnya */}
              <div className="fixed inset-0 bg-white pointer-events-none" style={{ animation: 'whiteFlash 0.9s ease-in-out forwards' }} />
            </div>
          )}

          {/* Tombol Pilihan Menu (Hanya bisa diklik pas Idle) */}
          {launchState === "idle" && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col gap-4 items-center z-10">
              <button
                onClick={() => handleLaunch("start")}
                className="block w-[220px] py-[14px] text-center bg-gradient-to-r from-[#874FFE] to-[#67339C] rounded-lg text-white font-bold text-[18px] tracking-[0.1em] uppercase shadow-[0_0_20px_#874FFE80] cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-[0_0_35px_#C4A9FF]"
              >
                Start Now
              </button>
              ...
              <button
                ...
                className="block w-[220px] py-[14px] text-center bg-[#120524]/80 border-2 border-[#874FFE]/50 rounded-lg text-[#C4A9FF] font-bold text-[16px] tracking-[0.1em] uppercase backdrop-blur-sm cursor-pointer transition-all duration-300 hover:bg-[#874FFE]/30 hover:border-[#C4A9FF] hover:text-white"
              >
                Instruction
              </button>
            </div>
      ...
      {launchState === "arrived" && (
        <div className="w-full px-4 flex items-center justify-center animate-[fadeIn_0.5s_ease-out]">
         {selectedFlow === "instruction" && <InstructionFlow onBack={handleBackToCockpit} onRegister={() => setSelectedFlow("start")} />}
          {selectedFlow === "start" && <RegistrationFlow onBack={handleBackToCockpit} />}
        </div>
      )}
    </div>
  );
