...
function SpeakerCard({
  ...
  return (
    <div
      className="group cursor-pointer [perspective:1000px] shrink-0 w-[264px]"
    ...
    >
      ...
      <div className="relative w-full h-[339px] transition-transform duration-500 ease-in-out [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
        ...
        <div
          className="absolute inset-0 bg-[#F9F5FF] border border-[#C4A9FF] rounded-xl flex flex-col items-center overflow-hidden p-3 [backface-visibility:hidden] [transform:rotateY(180deg)]">
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[1440px] px-4 md:px-8 lg:px-[120px] border-r border-l border-[#C4A9FF]">
        ...
        <div className="px-6 py-14 md:py-[80px] flex flex-col gap-[14px] items-start w-full border-r border-l border-[#C4A9FF] overflow-hidden">
          ...
          <div className="flex gap-[32px] items-center justify-start md:justify-center w-full pb-2 md:pb-0 md:flex-wrap">
            {speakers.map((s) => (
              <SpeakerCard key={s.id} speaker={s} onOpen={setSelected} />
            ))}
          </div>

          <p className="w-full text-center  font-normal text-[#C4A9FF] text-[16px] mt-2">
            Click to read full bio
          </p>
        </div>
      </div>
